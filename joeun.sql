...
ON DELETE CASCADE
;
-- ONDELETE [NOACTION, RESTRICT, CASCADE, SET NULL]
-- * NO ACTION  : 아무 행위도 안함.
-- * RESTRICT   : 자식 테이블의 데이터가 존재하면, 삭제 안 함
-- * CASCADE    : 자식 테이블의 데이터도 함꼐 삭제
...
ON DELETE [NO ACTION, RESTRICT, SET NULL]
;
...
SELECT e.EMP_ID 사원번호
      ...
      ,n.NATIONAL_NAME 국가명
      ,e.SALARY 급여
      ,TO_CHAR(e.HIRE_DATE,'YY/MM/DD') 입사일자
...
CREATE OR REPLACE VIEW VE_EMP_DEPT 
AS (
      SELECT E.EMP_ID 
            ,E.EMP_NAME 
            ,D.DEPT_TITLE 
...
SELECT dept_code
      ...
      , CASE 
            ...
            WHEN GROUPING(dept_code) = 1 AND GROUPING(job_code) = 1 THEN JOB_CODE || '의 합계'  
       END
FROM employee
WHERE dept_code IS NOT NULL
  AND job_code IS NOT NULL
