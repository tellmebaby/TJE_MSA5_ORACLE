...
BEGIN
    VI_NUM := 100;  -- 실행부
    VI_NUM := 1000;
    -- CO_NUM := 2000; -- 상수는 값을 최초에 한 번만 초기화하고 바꿀 수 없다.
...
-- 조회하여 1,000,000원 이상 2,000,000원 이하 이면 1을 출력하고
-- 만약에 2,000,001원 이상 3,000,000원 이하 이면 2 출력
-- 그렇지 않으면 3을 출력해 보셈
DECLARE
    -- 지정된 부서 D1
...
BEGIN
    FOR i IN 1..20
    LOOP
        CONTINUE WHEN MOD( i, 2 ) = 0;      -- 짝수 일 때, 건너 뜀
...
SELECT get_dept_title('D2')
FROM dual;

-- 사원번호 컬럼으로 모두의 부서명을 구하는 함수 생성
...
IS
    RESULT VARCHAR2(10);
    OUT_MANAGER_ID EMPLOYEE.MANAGER_ID%TYPE;
BEGIN
    -- 직원 타입 조회
    SELECT
        CASE  
...
/
-- emp id를 인자로 넘겨주면
-- 사원 구분을 '매니저', '사원' 으로 반화하는 함수를 정의해보세요.ADD
-- 함수명 : emp_type()
...
WHERE EMPLOYEE_ID = '102' AND JOB_ID = 'IT_PROG'
;
...
--
EXECUTE PRO_APP_EMP ('103', 'AC_MGR', '2023/01/01', '2028/12/31');

EXECUTE PRO_APP_EMP ('103', 'AC_ACCOUNT', '2024/01/01', '2026/12/31');
